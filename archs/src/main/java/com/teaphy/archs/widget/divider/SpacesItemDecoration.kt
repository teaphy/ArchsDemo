...
class SpacesItemDecoration(private val space: Int, private val spaceTop: Int = 0) : RecyclerView.ItemDecoration() {
	...
	override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
		...
		if (LinearLayoutManager.VERTICAL == layoutManager.orientation) {
			...
			when {
				// 第一个item
				parent.getChildLayoutPosition(view) == 0 -> outRect.set(space + spaceTop, 0,0, 0)
				// 最后一个Item
				parent.getChildLayoutPosition(view) == layoutManager.itemCount - 1 -> outRect.set(space, 0, space + spaceTop, 0)
